  ...
  return {
    ...
    team_create_agent: tool({
      ...
      args: {
        ...
        default_model: tool.schema.string().optional().describe('Model id in "providerID/modelID" form (e.g. "anthropic/claude-sonnet-4-6"). Omit to inherit the project\'s seeded default (matches whatever provider the sandbox has credentials for).'),
      },
      async execute(args, ctx): Promise<string> {
        const pid = getProjectIdForCtx(mgr, ctx)
        if (!pid) return 'Error: no session context.'
